...
const carInfo = function(id, year, type) {
    
    return `Car ${inventory[32].id} is a ${inventory[32].car_year} ${inventory[32].car_make}`;
}
...
let lastCar = function() {
    console.log("--Last Car--", inventory[inventory.length-1]);
}
...
for(let i = 0; i < inventory.length; i++) {
	...
	// console.log("--BMW & Audi--", JSON.stringify({BMWAndAudi}));
